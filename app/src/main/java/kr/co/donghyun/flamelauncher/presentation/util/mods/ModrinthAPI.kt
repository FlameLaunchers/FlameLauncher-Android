...
class ModrinthAPI {
    ...
    fun search(
        ...
        loader: String = "",           // facet: categories (모드로더. 비면 미적용)
        limit: Int = 20,
        offset: Int = 0,
    ): List<ModrinthSearchHit> {
        // facets: [["project_type:modpack"],["versions:1.20.1"],["categories:fabric"]] (AND 로 묶임)
        ...
        if (loader.isNotBlank()) facetGroups.add("[\"categories:${loader.lowercase()}\"]")
